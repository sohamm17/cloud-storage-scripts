...
labels = [#"UCLA to GoogleDrive (Direct)", "UCLA to Coldlake", "Coldlake to GoogleDrive", "UCLA to UMich", "UMich to GoogleDrive", 
	"Purdue to Dropbox (Direct)", "Purdue to UMich", "UMich to Dropbox", "Purdue to Dropbox (UMich-cut-through)",
...
for filesz in x:
	values = []
	with open("DropDirectUMich.txt") as f:
		for line in f:
			lineValues = line.split('\t')
			if lineValues[2] != stream or (int)(lineValues[1]) != filesz:
				continue
...
for filesz in x:
	values = []
	with open("DropUMichInter.txt") as f:
		for line in f:
			lineValues = line.strip().split(' ')
			if (int)(lineValues[0]) != filesz:
				continue
...
for filesz in x:
	values = []
	with open("DropUMichInterWorm.txt") as f:
		for line in f:
			lineValues = line.strip().split(' ')
			if (int)(lineValues[0]) != filesz:
				continue
