...
labels = [#"UCLA to GoogleDrive (Direct)", "UCLA to Coldlake", "Coldlake to GoogleDrive", "UCLA to UofA", "UofA to GoogleDrive", 
	"UCLA to One Drive (Direct)", "UCLA to UofA", "UofA to One Drive", "UCLA to One Drive (UofA-cut-through)",
	#"UCLA to OneDrive (Direct)", "UCLA to Coldlake", "Coldlake to OneDrive", "UCLA to UofA", "UofA to OneDrive"
	]
stream = 'upload'
bar_width = 0.50
...
for filesz in x:
	values = []
	with open("../GoogleDriveInterUCLAToUofA.txt") as f:
		for line in f:
			lineValues = line.strip().split(' ')
			if (int)(lineValues[0]) != filesz:
				continue
...
for filesz in x:
	values = []
	with open("../GoogleDriveInterNodeUCLAUofA.txt") as f:
		for line in f:
			lineValues = line.split('\t')
			if lineValues[2] != stream or (int)(lineValues[1]) != filesz:
				continue
...
for filesz in x:
	values = []
	with open("OneUofAInter_UCLA.txt") as f:
		for line in f:
			lineValues = line.strip().split(' ')
			if (int)(lineValues[0]) != filesz:
				continue
...
for filesz in x:
	values = []
	with open("OneUofARemoteUCLA.txt") as f:
		for line in f:
			lineValues = line.split('\t')
			if lineValues[2] != stream or (int)(lineValues[1]) != filesz:
				continue
...
#plt.show()
plt.savefig("./ucla_one_cut_uofa.pdf", bbox_inches='tight', dpi=60000)
