...
labels = [#"UCLA to GoogleDrive (Direct)", "UCLA to Coldlake", "Coldlake to GoogleDrive", "UCLA to UMich", "UMich to GoogleDrive", 
	"UCLA to Dropbox (Direct)", "UCLA to UBC", "UBC to Dropbox", "UCLA to Dropbox (UBC-cut through)",
...
for filesz in x:
	values = []
	with open("DropDirectUCLAUBC.txt") as f:
		for line in f:
			lineValues = line.split('\t')
			if lineValues[2] != stream or (int)(lineValues[1]) != filesz:
				continue
...
for filesz in x:
	values = []
	with open("DropUBCInterUCLA.txt") as f:
		for line in f:
			lineValues = line.strip().split(' ')
			if (int)(lineValues[0]) != filesz:
				continue
...
for filesz in x:
	values = []
	with open("DropUBCRemoteUCLA.txt") as f:
		for line in f:
			lineValues = line.split('\t')
			if lineValues[2] != stream or (int)(lineValues[1]) != filesz:
				continue
...
for filesz in x:
	values = []
	with open("DropUBCInterWorm.txt") as f:
		for line in f:
			lineValues = line.strip().split(' ')
			if (int)(lineValues[0]) != filesz:
				continue
...
ax.legend(rects, labels, prop={'size':'24'},loc='upper center', bbox_to_anchor=(0.38, 0.97), ncol=1, fancybox=True, shadow=True)
