...
labels = [#"UCLA to GoogleDrive (Direct)", "UCLA to Coldlake", "Coldlake to GoogleDrive", "UCLA to UMich", "UMich to GoogleDrive", 
	"UBC to Google Drive (Direct)", "UBC to UofA", "UofA to Google Drive", "UBC to Google Drive (UofA-cut-through)",
...
for filesz in x:
	values = []
	with open("GoogDirectUofA.txt") as f:
		for line in f:
			lineValues = line.split('\t')
			if lineValues[2] != stream or (int)(lineValues[1]) != filesz:
				continue
...
for filesz in x:
	values = []
	with open("GoogUofAInter.txt") as f:
		for line in f:
			lineValues = line.strip().split(' ')
			if (int)(lineValues[0]) != filesz:
				continue
...
for filesz in x:
	values = []
	with open("GoogUofARemote.txt") as f:
		for line in f:
			lineValues = line.split('\t')
			if lineValues[2] != stream or (int)(lineValues[1]) != filesz:
				continue
...
for filesz in x:
	values = []
	with open("GoogUofAInterWorm.txt") as f:
		for line in f:
			lineValues = line.strip().split(' ')
			if (int)(lineValues[0]) != filesz:
				continue
